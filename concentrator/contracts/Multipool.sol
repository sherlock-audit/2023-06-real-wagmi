...
contract Multipool is Ownable, ERC20 {
     ...
     * They should be used with caution, as they do not take into account the current accumulated fees.
     * Tthe amount of pending fees per share should be added to the userRewardDebt variable.
     */
    uint256 public feeGrowthInside0LastX128accPerShare;
    uint256 public feeGrowthInside1LastX128accPerShare;
    ...
    /**
     * @notice This function collect fees and update fee Growth Per Share.
     * @return feeGrowthInside0PerShare The fee growth inside the first Vault per share after the earning.
     * @return feeGrowthInside1PerShare The fee growth inside the second Vault per share after the earning.
     */
    function earn()
        external
        returns (uint256 feeGrowthInside0PerShare, uint256 feeGrowthInside1PerShare)
    {
        _earn();
        feeGrowthInside0PerShare = feeGrowthInside0LastX128accPerShare;
        feeGrowthInside1PerShare = feeGrowthInside1LastX128accPerShare;
    }
    ...
    function _deposit(
        ...
        _pay(token0, address(this), owner(), (fee0 * protocolFeeWeight) / MAX_WEIGHT_UINT256);
        _pay(token1, address(this), owner(), (fee1 * protocolFeeWeight) / MAX_WEIGHT_UINT256);
        // all fee(including protocol fee)
        if (_totalSupply > 0) {
            feeGrowthInside0LastX128accPerShare += FullMath.mulDiv(
                fee0,
                FixedPoint128.Q128,
                _totalSupply
            );

            feeGrowthInside1LastX128accPerShare += FullMath.mulDiv(
                fee1,
                FixedPoint128.Q128,
                _totalSupply
            );
    ...
    /**
     * @dev Deposit function for adding liquidity to the pool
     * @param amount0Desired amount of token0 desired to deposit
     * @param amount1Desired amount of token1 desired to deposit
     * @param amount0Min minimum amount of token0 required
     * @param amount1Min minimum amount of token1 required
     * @return lpAmount returns the amount of liquidity tokens created
     * @return feeGrowthInside0PerShare The fee growth inside the first Vault per share after the depositing.
     * @return feeGrowthInside1PerShare The fee growth inside the second Vault per share after the depositing.
     * */
    function deposit(
        ...
        returns (
            uint256 lpAmount,
            uint256 feeGrowthInside0PerShare,
            uint256 feeGrowthInside1PerShare
        )
    {
        uint256 _totalSupply = totalSupply();
        Slot0Data[] memory slots;
     ...
     * @param amount0OutMin The minimum amount of token0 that must be received on withdrawal.
     * @param amount1OutMin The minimum amount of token1 that must be received on withdrawal.
     * @dev This function will burn `lpAmount` of liquidity pool tokens of the caller and send back `withdrawnAmount0`
     *      and `withdrawnAmount1` amounts of token0 and token1 respectively. It uses Uniswap V3's burn function
     *      to withdraw the liquidity and calculates the amounts of token0 and token1 owed to the caller based on their
     *      share of the total liquidity. It then collects the accumulated fees for the position before burning and
     *      withdrawing liquidity. Finally, it transfers the withdrawn tokens to the caller and emits the Withdraw
     *      event.
     * @return feeGrowthInside0PerShare The fee growth inside the first Vault per share after the withdrawing.
     * @return feeGrowthInside1PerShare The fee growth inside the second Vault per share after the withdrawing.
     */
    function withdraw(
        uint256 lpAmount,
        uint256 amount0OutMin,
        uint256 amount1OutMin
    ) external returns (uint256 feeGrowthInside0PerShare, uint256 feeGrowthInside1PerShare) {
        uint256 _totalSupply = totalSupply();

        (uint256 withdrawnAmount0, uint256 withdrawnAmount1) = _withdraw(lpAmount, _totalSupply);
    ...
    /**
     * @dev This function returns the current reserves
    ...
    function _getReserves(
        ...
        reserve0 += (feeGrowth0 * feeGrowthWeight) / MAX_WEIGHT_UINT256;
        reserve1 += (feeGrowth1 * feeGrowthWeight) / MAX_WEIGHT_UINT256;
    }
    ...
    function _checkDeviation(
        ...
        uint amountIn,
        uint amountOut
    ) internal view returns (uint256 swappedOut) {
    ...
    function rebalanceAll(RebalanceParams calldata params) external nonReentrant {
        ...
        uint256 swappedOut = _checkDeviation(params.zeroForOne, amountIn, amountOut);
