...
contract Factory is Ownable, IFactory {
    ...
    constructor(address _underlyingV3Factory, address _multipoolCode) {
    ...
    function createMultipool(
        ...
        assembly {
            multipool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(multipool != address(0), "Failed on deploy");

        multipools[token0][token1] = multipool;

        strategy.setMultipool(multipool);
